...
class MainStore {
  ...
  export(type) {
    if (this.indexValue === null) {
      ...
      this.loading = true;
      let fields = null;
      if (this.fieldValue === null) fields = this.fieldList;
      else fields = this.fieldValue;
      ...
      post("/export", {
        ...
        .then(response => {
          ...
          if (type === "csv") {
            fileDownload(
              response.data,
              this.indexValue + "-" + this._generateID() + ".csv"
            );
          } else if (type === "mongo") {
            fileDownload(
              response.data,
              this.indexValue + "-" + this._generateID() + "-instructions.txt"
            );
          }
  ...
  @action
  timeRangeToggle() {
    if (this.isTimeRangeVisible) this.isTimeRangeVisible = false;
    else this.isTimeRangeVisible = true;
  }
